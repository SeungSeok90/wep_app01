...
export default function AttendClient({ registration, event, template }: Props) {
  ...
  async function handleCheckIn() {
    ...
    if (res.ok) { setCheckedIn(true); setCheckedInAt(data.checked_in_at) }
    setLoading(false)
  }
    ...
    <>
      ...
      <div className="no-print min-h-screen bg-slate-50 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          <div className={`text-center mb-6 px-4 py-2 rounded-full text-sm font-medium ${
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-4">
            <div className="bg-indigo-600 px-6 py-4 text-white">
              ...
              {event.event_date && (
                <p className="text-xs text-indigo-200">{new Date(event.event_date).toLocaleString('ko-KR')}</p>
              )}
            </div>
            <div className="p-6 flex items-center gap-4">
              <div className="flex-1">
                <p className="text-2xl font-bold text-slate-900">{registration.name}</p>
                {registration.company && <p className="text-slate-600 mt-1">{registration.company}</p>}
          ...
          <div className="flex flex-col gap-3">
            {!checkedIn ? (
              <button
                ...
                disabled={loading}
                className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium py-3.5 rounded-xl transition-colors"
              >
                {loading ? '처리 중...' : '✅ 출석 체크'}
              </button>
            ) : (
              <div className="w-full bg-emerald-50 border border-emerald-200 text-emerald-700 font-medium py-3.5 rounded-xl text-center">
                출석 완료
              </div>
            )}
            <button
              onClick={handlePrint}
              className="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-medium py-3.5 rounded-xl transition-colors"
            >
              🖨️ 네임택 인쇄
            </button>
          </div>
