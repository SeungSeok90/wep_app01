...
export default async function AdminPage({
  ...
  // 행사 목록 + 전체 등록 데이터 병렬 조회
  let eventsQuery = supabase.from('events').select('*').order('event_date', { ascending: false })
  if (query) {
    eventsQuery = eventsQuery.or(`name.ilike.%${query}%,location.ilike.%${query}%,organizer.ilike.%${query}%`)
  }
  ...
  for (const r of regs) {
    ...
    if (r.attendance_type === 'online') s.online++
    else s.offline++
    statsMap.set(r.event_id, s)
  }

  const globalStats = [
    { label: '전체 행사', value: eventList.length, sub: `검색 결과 ${eventList.length}개`, color: 'text-slate-800' },
    { label: '전체 등록', value: regs.length, sub: '명', color: 'text-indigo-600' },
    { label: '전체 출석', value: totalCheckedIn, sub: '명', color: 'text-emerald-600' },
    { label: '현장 등록', value: totalWalkIn, sub: '명 (당일)', color: 'text-amber-600' },
    { label: '전체 출석률', value: `${overallRate}%`, sub: `${totalCheckedIn} / ${regs.length}`, color: overallRate >= 80 ? 'text-emerald-600' : overallRate >= 50 ? 'text-amber-600' : 'text-red-500' },
  ]
      ...
      <div className="flex">
        <aside className="w-56 min-h-screen bg-slate-900 text-white flex flex-col">
          ...
          <nav className="flex flex-col gap-1 p-4 flex-1">
            <span className="px-4 py-2.5 rounded-lg text-sm bg-indigo-600 text-white">개요 / 행사 관리</span>
          </nav>
        ...
        <main className="flex-1 p-8 overflow-x-auto">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">전체 행사 현황 한눈에 보기</p>
            </div>
            <Link href="/admin/events/new" className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">
              + 새 행사 만들기
            </Link>
          </div>
                    ...
                    return (
                      <tr key={event.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-4">
                          <Link href={`/admin/events/${event.id}`} className="font-medium hover:text-indigo-600 hover:underline transition-colors">
                            {event.name}
                          </Link>
                        ...
                        {/* D-Day */}
                        <td className="px-5 py-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${eventStatus.color}`}>
                            {eventStatus.label}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          ...
                          {event.type === 'hybrid' && s.total > 0 && (
                            <p className="text-xs text-slate-400">현장 {s.offline} / 온라인 {s.online}</p>
                          )}
                        </td>
                        ...
                        {/* 현장 등록 */}
                        <td className="px-5 py-4">
                          <p className={`font-semibold ${s.walkIn > 0 ? 'text-amber-600' : 'text-slate-300'}`}>
                            {s.walkIn}명
                          </p>
                        </td>
                        ...
                        <td className="px-5 py-4 w-32">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-100 rounded-full h-1.5">
                              <div
                                className={`h-1.5 rounded-full transition-all ${rate >= 80 ? 'bg-emerald-500' : rate >= 50 ? 'bg-amber-400' : 'bg-slate-300'}`}
                                style={{ width: `${rate}%` }}
                              />
                            </div>
                        ...
                        {/* 등록 상태 */}
                        <td className="px-5 py-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${regStatus.color}`}>
                            {regStatus.label}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex gap-2 items-center">
                            <Link href={`/admin/checkin/${event.id}`} className="text-emerald-600 hover:text-emerald-800 text-xs font-medium transition-colors">
                              출석
                            </Link>
                            <Link href={`/admin/events/${event.id}`} className="text-indigo-500 hover:text-indigo-700 text-xs transition-colors">
                              편집
                            </Link>
                            <a href={`/${event.slug}`} target="_blank" className="text-slate-400 hover:text-slate-600 text-xs transition-colors">
                              폼
                            </a>
